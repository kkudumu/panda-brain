...
describe('CLI User Scenarios (via createProgram)', { timeout: 30_000 }, () => {
  let store: FtmStore;
  ...
  beforeEach(async () => {
    ...
    // Mock ora to avoid spinner noise in tests
    vi.mock('ora', () => ({
  ...
  it('ftm status — shows the daemon state', async () => {
    ...
    // commander uses process.argv style: [node, script, ...args]
  ...
  it('ftm <task> — submits a task and receives progress events', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const program = createProgram();
    ...
    // Should show progress events and eventually completion
    await vi.waitFor(() => {
