...
import type { WsMessage, WsResponse, MachineState } from '../shared/types.js';
...
async function connectToDaemon(port: number = 4040): Promise<WebSocket> {
...
async function sendAndWait(ws: WebSocket, type: string, payload: Record<string, unknown> = {}): Promise<WsResponse> {
  return new Promise((resolve, reject) => {
...
program
  ...
  .action(async () => {
    ...
    const { getDataDir, getDbPath } = await import('../daemon/config.js');
...
program
  ...
  .action(async () => {
    ...
    const { getConfigPath, getDataDir, ensureDataDir } = await import('../daemon/config.js');
