...
### Listens To
- `task_received` — begin ideation when panda-mind routes an incoming task for exploration
  - Expected payload: `{ task_description, plan_path, wave_number, task_number }`

## Config Read

Before dispatching any agents, read `~/.claude/panda-config.yml`:
...
Every turn, read `references/agent-prompts.md` and spawn **3 parallel agents** (subagent_type: general-purpose, model: from panda-config `planning` profile). Each agent gets:
...
- **panda-brainstorm** (this): Idea exploration with live research. User is figuring out WHAT to build.
- **superpowers:brainstorming**: Design/spec work. User knows what they're building, needs HOW.

If user already completed superpowers:brainstorming, point to panda-executor instead. If user explicitly invokes this skill, always run it.

---

## Session State (for panda-pause/resume)

When paused, the following state must be capturable so panda-resume can pick up exactly where you left off:
...
This state is what panda-pause captures and panda-resume restores. Keep it current as you go.
